		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("TravelAgencyID") === oObject.TravelAgencyID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].TravelAgencyID;
									} else {
										sCurrentId = aItemsNotInTheList[0].TravelAgencyID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("TravelAgencyID"),
						title: oBindingContext.getProperty("City")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "TravelAgencies (0)"
						}),
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'TravelAgencies (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("City");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all TravelAgencies to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "TravelAgencies (" + iExpectedLength + ")"
								}),
								success: function() {
