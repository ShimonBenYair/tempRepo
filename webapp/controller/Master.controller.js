...
sap.ui.define([
	...
	return BaseController.extend("kj.controller.Master", {
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("City", FilterOperator.Contains, sQuery)];
			} else {
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							TravelAgencyID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("TravelAgencyID"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				TravelAgencyID: encodeURIComponent(oItem.getBindingContext().getProperty("TravelAgencyID"))
			}, bReplace);
		},
